...
let countDown;
 ...
 DURATION=Number(document.getElementById("MINUTES").getAttribute("minute"));

 $("#MINUTES").text(DURATION);

 function inMilisecond(val) {
 //convertir minute en millisecondes
 val = Number(val);
 return val * 60000;
 }
...
function compteur(Temps){
 ...
 if(distance>0){
  ...
  (day_val = Math.floor(distance / day)),
  (hours_val = Math.floor((distance % day) / hour)),
  (minutes_val = Math.floor(
  (distance % hour) / minute
  )),
  (second_val = Math.floor(
 ...
 if (second_val < 10) {
  ...
  (document.getElementById("MINUTES").innerText = minutes_val),
  (document.getElementById("SECONDES").innerText = second_val);


 }else{

  window.clearInterval(counter_clock);
  $("#Start_proclamation").addClass("ico_appear");
  $("#Pause_proclamation").addClass("ico_appear");
  $("#Stop_proclamation").addClass("ico_appear");


  $(".conterenduproclamation").addClass("activate_compte_rendu");
...
function second_Milli(val) {
  //convertir seconde en millisecondes
  val = Number(val);
  return val * 1000;
}
function inMilisecond(val) {
...
function start_clock(temps) {

    countDown = new Date().getTime() + inMilisecond(temps);
    counter_clock= window.setInterval(function () {compteur(countDown);}, 0);
    console.log("Gooo");
}


function continous_clock() {

    // recuperation du temps restant en miliseconde
    let tempo= inMilisecond($("#MINUTES").text()) +  second_Milli($("#SECONDES").text());
    countDown = new Date().getTime() + tempo;
    counter_clock= window.setInterval(function () {compteur(countDown);}, 0);
    console.log("continous_clock");
}

// responsiveVoice.speak($(".question_val").text());

$("#Start_proclamation").on("click", function () {

      $("#Pause_proclamation").removeClass("ico_appear");
      $("#Stop_proclamation").removeClass("ico_appear");
      $("#Start_proclamation").addClass("ico_appear");

      if(init){
        countDown = new Date().getTime() + inMilisecond(DURATION);
      window.clearInterval(counter_clock);
      }
...
$("#Pause_proclamation").on("click", function () {

  $("#Pause_proclamation").addClass("ico_appear");
  $("#Start_proclamation").removeClass("ico_appear");
...
$("#Stop_proclamation").on("click", function () {
  $("#Pause_proclamation").addClass("ico_appear");
  $("#Start_proclamation").removeClass("ico_appear");
